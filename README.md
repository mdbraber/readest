...
| **KOReader plugin** | `apps/readest.koplugin/` — custom-server support, a single-flight token refresh with graduated retries while Wi-Fi settles after wake, the same authoring-time position rule as the app, and a push on close that bypasses the debounce. Pulls on open, wake and reconnect come from upstream's auto sync | [6854eb0](https://github.com/mdbraber/readest/commit/6854eb0b), [f3149a8](https://github.com/mdbraber/readest/commit/f3149a87) |
| **Reading-position sync** | Positions sync by "most recently authored wins" rather than "furthest wins", so going back on one device carries over to the others. Each config records when its position was authored (`progressUpdatedAt`); opening a book, changing settings or adopting another device's position never counts as a new move. The server merges the position on that timestamp independently of the rest of the config, in both directions, and its reply to every page-turn push carries any newer position from another device, so no extra requests are needed. Builds on upstream's pull retries, resume pulls and close flush | [1de24f4](https://github.com/mdbraber/readest/commit/1de24f49), [700ae30](https://github.com/mdbraber/readest/commit/700ae301) |
| **Offline PWA** | Service worker rescues any page navigation offline: the library page falls back to its cached copy, other pages to the offline page (never another route's page). The web build uses webpack (`next build --webpack`) because Serwist only generates the service worker there | [1de24f4](https://github.com/mdbraber/readest/commit/1de24f49), [0e4d22e](https://github.com/mdbraber/readest/commit/0e4d22e0) |
...
| **Self-host build scripts** | `scripts/patch-tauri-selfhost.mjs` patches Tauri config for self-host builds; `scripts/scan-public-fork-safety.sh` runs before every push (this repository is public) and blocks secrets, real hostnames and signing identities | [5c19b4f](https://github.com/mdbraber/readest/commit/5c19b4f0), [ae86e0a](https://github.com/mdbraber/readest/commit/ae86e0a1) |
| **Self-host Docker image** | Runs the full Next.js server (`pnpm start-web`) instead of upstream's standalone tree, with the service worker built in. Marked `SELF_HOSTED` at build time so large book uploads get the self-hosted request-body limit. The fork's database migration is `900_fork_book_configs_progress_updated_at.sql`, numbered to stay clear of upstream's | [5c19b4f](https://github.com/mdbraber/readest/commit/5c19b4f0), [700ae30](https://github.com/mdbraber/readest/commit/700ae301) |
...
| **CI/CD removed** | All upstream GitHub Actions workflows removed, including ones upstream adds later (currently also the Nix and AppImage workflows) — not needed on a personal fork | [bd79033](https://github.com/mdbraber/readest/commit/bd79033d) |
| **Android updates per architecture** | The in-app updater picks the APK for the device's CPU (arm64, armv7, x86_64, x86) and falls back to the universal APK | [cda967a](https://github.com/mdbraber/readest/commit/cda967a5) |
| **iOS dark icon** | `scripts/patch-ios-dark-icon.mjs` adds a dark-appearance app icon that survives `tauri ios init` | [6e59095](https://github.com/mdbraber/readest/commit/6e59095f) |
